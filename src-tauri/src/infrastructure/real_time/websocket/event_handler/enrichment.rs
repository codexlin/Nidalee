...
fn apply_confirmed_bot_identity(
    ...
    cached_player.position = (!live_player.position.trim().is_empty()).then(|| live_player.position.clone());
...
impl WsEventHandler {
    ...
    pub(super) async fn backfill_enemy_team_data(&self, generation: u64) -> Result<()> {
        ...
        for live_player in enemy_live_players.iter().filter(|player| player.is_bot) {
            ...
            let Some(enemy_player) = team_analysis
                .enemy_team
                .iter_mut()
                .find(|player| player.champion_id == Some(champion_id))
            else {
                log::warn!(
        ...
        for live_player in enemy_live_players {
            ...
            let enemy_index = team_analysis.enemy_team.iter().position(|p| {
            ...
            let enemy_index = match enemy_index {
                Some(index) => index,
                None => {
                    log::warn!(
            ...
            enemy_player.position = (!live_player.position.trim().is_empty()).then(|| live_player.position.clone());
    ...
    async fn build_player_data(
        ...
        let mut player_data = crate::shared::types::PlayerAnalysisData {
            ...
            position: (!live_player.position.trim().is_empty()).then(|| live_player.position.clone()),
            solo_rank: None,
            flex_rank: None,
