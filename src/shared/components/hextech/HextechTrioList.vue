...
const ringFor = (item: HextechGuideAugment) =>
  rarityRing[normalizeRarityKey(item.rarityName, item.rarityDisplayName)]
...
const routeTitle = (trio: HextechGuideTrio) =>
  `${routeNames(trio)} · ${formatHextechPct(trio.winRate)} 胜率`
...
<template>
  <div v-if="compact && shownTrios.length" class="space-y-1">
    <article
  ...
  <div v-else class="space-y-1.5">
    ...
    >
      ...
      <p class="hidden shrink-0 items-center justify-end gap-3 text-sm tabular-nums sm:flex">
        <span class="w-12 text-right font-medium text-sky-600 dark:text-sky-400">{{ formatHextechPct(trio.winRate) }}</span>
