...
const rarityBoards = computed(() => {
  const source = compact.value
    ? tabGroups.value.filter(
        (group) => group.key === (tabGroups.value.some((item) => item.key === activeKey.value) ? activeKey.value : tabGroups.value[0]?.key)
      )
  ...
  return source.map((group) => {
    const bands = groupAugmentsByTier(group.items).map((band) => {
      ...
      const limit = band.tier === 3 || band.tier === 4 || band.tier === 0 ? (compact.value ? 8 : 10) : compact.value ? 4 : 6
...
<template>
  <div v-if="allGroups.length" :class="compact ? 'space-y-2' : 'space-y-3'">
    ...
    <div v-if="tabGroups.length > 1" class="flex gap-0.5 rounded-lg bg-black/25 p-0.5" :class="!compact && 'hidden'">
      <button
        ...
        :class="
          activeKey === group.key ? `${rarityTone[group.key]} bg-white/10` : 'text-muted-foreground hover:text-foreground'
        "
    ...
    <div v-else class="grid grid-cols-1 gap-3 lg:grid-cols-3">
      <section v-for="group in rarityBoards" :key="group.key" class="min-w-0">
        ...
        <div v-if="group.bands.length" class="space-y-3">
