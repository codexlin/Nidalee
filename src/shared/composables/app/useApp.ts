  ...
  onMounted(async () => {
    ...
    try {
      ...
      try {
        ...
        const shortcut =
          !requested || requested === 'F8' || requested === 'Home' ? DEFAULT_OVERLAY_SHORTCUT : requested
        const saved = await invoke<string>('set_augment_overlay_shortcut', { shortcut })
        settingsStore.setAugmentOverlayShortcut(saved)
        const ocrEnabled = await invoke<boolean>('set_augment_overlay_ocr_enabled', {
          enabled: settingsStore.augmentOverlayOcrEnabled
        })
        settingsStore.setAugmentOverlayOcrEnabled(ocrEnabled)
      } catch (error) {
