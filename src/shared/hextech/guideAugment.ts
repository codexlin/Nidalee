...
export function groupAugmentsByTier(items: HextechGuideAugment[]): HextechTierGroup[] {
  ...
  for (const item of items) {
    const key: HextechTierKey =
      item.tier === 1 || item.tier === 2 || item.tier === 3 || item.tier === 4 ? item.tier : 0
    const list = buckets.get(key) ?? []
    list.push(item)
    buckets.set(key, list)
  }
...
export function hasSampledWinRate(item: HextechGuideAugment): boolean {
  if (item.winRate == null || Number.isNaN(item.winRate)) return false
  if (item.games == null && item.winRate === 0) return false
  return true
}
